...
<head>
  ...
  <style>
    ...
    .whiteBox {background-color: white;	position: absolute;	width: 100% !important;	top: 350px;	height:100%; left: 0px;}
    .scrollBox { position: absolute; width: 92%; padding-left: 4%; overflow-y: auto; overflow-x: hidden; top: 880px; height: 99%; left: 0px; bottom:10px;margin-right: -100px; padding-right: 100px; }
  ...
  function documentLoad(){
    ...
    else if (localStorage.getItem("shared") != undefined && localStorage.getItem("shared") != "undefined"){
      var index = localStorage.getItem("shared");
      var splitArray = strArray[index].split("ppp");
      localStorage.setItem("shared", undefined);
      window.location = 'shareFile.php?splitArray=' + splitArray;
    }

    <?php if ($_SESSION['filter1'] == true){ ?>
      $('input[name=f1]').click();
    <?php } ?>
    <?php if ($_SESSION['filter2'] == true){ ?>
      $('input[name=f2]').click();
    <?php } ?>
  ...
  function sort(type){
    ...
    document.getElementById('hidden').value = type;
  	...
  	if($('input[name=f1]').is(':checked')){
  		document.getElementById('hidden1').value = true;
    }
  	if($('input[name=f2]').is(':checked')){
  ...
  function saveToTable(idNumber){
    localStorage.setItem("shared", idNumber);
    location.reload();
  }
...
<body onload="documentLoad()">
  ...
  <div class="whiteBox">
    ...
    <div class="sortMenu" style="padding-left: 4%; padding-right:4%;">
      <p>Sort By:</p>
      <form action="previousInfrastructure.php" method="get">
        <input type="submit" class="sortBttn" onclick="sort('date');" value="Date Created" ><input type="submit" class="sortBttn" onclick="sort('cost');" value="Cost"><input type="submit" class="sortBttn" onclick="sort('service');" value="Service">
        <input type="hidden" id="hidden" name="hidden" value="">
        <input type="hidden" id="webpage" name="webpage" value="view">
        <p>Select Fields to View:</p>
        <table>
          <tr>
            <td><input type="checkbox" id="check-box" name="f1" value="ID" ><label>ID</label></td>
          </tr>
          <tr>
            <td><input type="checkbox" id="check-box" name="f2" value="Cost" ><label>Cost</label></td>
          </tr>
          <tr>
  ...
  <div class="scrollBox">
    ...
    <?php
      ...
      if ($sortValue == 'cost'){
        $sqlTwo = "SELECT * FROM infrastructure WHERE USERNAME = '{$userDisplay}' ORDER BY COST";
      }
      else if ($sortValue == 'date'){
        $sqlTwo = "SELECT * FROM infrastructure WHERE USERNAME = '{$userDisplay}' ORDER BY ID";
      }
      else if ($sortValue == 'service'){
        $sqlTwo = "SELECT * FROM infrastructure WHERE USERNAME = '{$userDisplay}' ORDER BY SERVICE";
      }
      else { //DEFAULT
        $sqlTwo = "SELECT * FROM infrastructure WHERE USERNAME = '{$userDisplay}' ORDER BY ID";
      }
      ...
      while(oci_fetch_array($stmtTwo))
      {
        ...
        $tempStr = 'view'.'ppp'.$id.'ppp'.$cost.'ppp'.$username.'ppp'.$service.'ppp'.$ram.'pppnullppp'.$vm.'ppp'.$cpu.'ppp'.$printPackages;
        ...
        if($_SESSION['filter1'] != ""){
          if (($backgroundCount % 2) == 0) {
            $backgroundColor = "background-color: #ebf6f9;";
          }
          else {
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">ID</th>
            <td style="'.$backgroundColor.'">'.$id.'</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter2'] != ""){
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">Cost</th>
            <td style="'.$backgroundColor.'">$'.$cost.'</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter3'] != ""){
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">Username</th>
            <td style="'.$backgroundColor.'">'.$username.'</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter4'] != ""){
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">Service</th>
            <td style="'.$backgroundColor.'">'.$service.'</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter5'] != ""){
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">RAM</th>
            <td style="'.$backgroundColor.'">'.$ram.'GB</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter8'] != ""){
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">VM</th>
            <td style="'.$backgroundColor.'">'.$vm.'</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter9'] != ""){
          ...
          $output[] = '
          <tr>
            <th style="width:15%;'.$backgroundColor.'">CPU</th>
            <td style="'.$backgroundColor.'">'.$cpu.'</td>
          </tr>';
          $backgroundCount++;
        }
        ...
        if($_SESSION['filter10'] != ""){
          if ($printPackages != ""){
            ...
            $output[] = '
            <tr>
              <th style="width:15%;'.$backgroundColor.'">Packages</th>
              <td style="'.$backgroundColor.'">'.$printPackages.'</td>
            </tr>';
            $backgroundCount++;
          }
        }
        ...
        if ($_SESSION['filter3'] != "" || $_SESSION['filter4'] != "" || $_SESSION['filter5'] != "" || $_SESSION['filter8'] != "" || $_SESSION['filter9'] != "" || $_SESSION['filter10'] != ""){
          ...
          <tr>
            ...
            <td style="text-align: right;">
              <input type="submit" style="text-align:center; width:150px; margin-bottom:5px;" class="submitBttn" value="Share" name="share" onclick="saveToTable('.$idCounter.')"><br>
